...
para = np.loadtxt(txt_folder + 'params_i5.txt')
...
# C12, O, I 
C12toO = se.exp(x[2]) * se.exp(se.exp(x[8]) * v)
OtoC12 = se.exp(x[3]) * (se.exp(x[0]) * se.exp(-se.exp(x[1]) * v)) / (1 + se.exp(x[0]) * se.exp(-se.exp(x[1]) * v))
OtoI = se.exp(x[4]) * se.exp(se.exp(x[5]) * v) / (1 + se.exp(x[0]) * se.exp(-se.exp(x[1]) * v))
ItoO = se.exp(x[6]) * se.exp(-se.exp(x[7]) * v)
conductance = se.exp(x[9]) / (1 + se.exp(x[0]) * se.exp(-se.exp(x[1]) * v))
...
# Write in matrix form taking y = ([C3O], [I])^T
# RHS = A * y + B
