...
para = np.loadtxt(txt_folder + 'params_i6.txt')
...
# C12, O, I 
C12toO = se.exp(x[1]) * se.exp(se.exp(x[7]) * v) 
OtoC12 = se.exp(x[2]) * se.exp(-se.exp(x[0]) * v)
OtoI = se.exp(x[3]) * se.exp(se.exp(x[4]) * v) 
ItoO = se.exp(x[5]) * se.exp(-se.exp(x[6]) * v)
conductance = se.exp(x[8])
...
iv, svals = InitialVelocity(para, funcs.S1_obs_vector, funcs.Avv, par, args.invert, args.eig)

if args.plot:
    plot = DiagnosticPlots(par, funcs.SimulateCurrent)
    plot.eigenval_spectrum_single(svals, figx=2.5, figy=4, grid=False)
...
if args.plot:
    ...
    # plot.geodesics_2D_contour_plot(funcs.rhs_obs_vector, geodesic_paths, 7, 9, xlims=[-9, -2.2], ylims=[-3, -1.9], \
    #     figx=3, figy=2.5)
