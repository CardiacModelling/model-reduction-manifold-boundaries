...
def main():
    ...
    par.observed_variable = obs_var - 1
    ...
    # C12, O, I 
    C12toO = se.exp(x[2]) * se.exp(x[8] * v)
    OtoC12 = se.exp(x[3]) * (se.exp(x[0]) * se.exp(-x[1] * v)) / (1 + se.exp(x[0]) * se.exp(-x[1] * v))
    OtoI = se.exp(x[4]) * se.exp(x[5] * v) / (1 + se.exp(x[0]) * se.exp(-x[1] * v))
    ItoO = se.exp(x[6]) * se.exp(-x[7] * v)
