...
para = np.loadtxt(txt_folder + 'params_i8.txt')
...
# C12, O
C12toO = se.exp(x[1]) * se.exp(se.exp(x[5]) * v)
OtoC12 = se.exp(x[2]) * se.exp(-se.exp(x[0]) * v) / (1 + se.exp(x[3]) * se.exp(se.exp(x[4]) * v))
conductance = se.exp(x[6]) / (1 + se.exp(x[3]) * se.exp(se.exp(x[4]) * v))

rates = [C12toO, OtoC12]
rates_str = ['C12toO', 'OtoC12']
states = ['C12', 'O'] # C12 is calculated from the other states

rhs = [ C12toO * (1 - y[0]) - OtoC12 * y[0] ] 
