...
para = np.loadtxt(txt_folder + 'params_i3.txt')
...
# C12, C3, O, I
C12toC3 = se.exp(x[4]) * se.exp(se.exp(x[10]) * v) 
C3toC12 = se.exp(x[5])
...
if args.plot:
    ...
    singular_values = np.loadtxt(txt_folder + 'geodesic_svals_' + iter_str + '.txt')
    ...
    plot.eigenval_spectrum_double(singular_values, grid=False, figx=3, figy=2.5)
    plot.geodesics(geodesic_taus, geodesic_paths, geodesic_velocities)
    plot.geodesics_2D_plot(geodesic_paths, 1, 2, grid=False, figx=3, figy=2.5)
    plot.geodesics_2D_contour_plot(funcs.rhs_obs_vector, geodesic_paths, 1, 2, xlims=[-5, -2], ylims=[-10, -3], \
        figx=3, figy=2.5)
