...
parser.add_argument('--figsize1', type=float, nargs='+', default=[6, 2.7], \
                    help='Figure size in x and y, e.g. --figsize1 2.5 3.5')
parser.add_argument('--figsize2', type=float, nargs='+', default=[3, 4], \
...
for n, model in enumerate(models):
    ...
    ax11.plot(v1, g1n, label=model_str, color=cmap(norm(n)))
    if n == 0:
        ax11.scatter(exp_ss_v,exp_ss_in, s=100, facecolors='none', edgecolors='red', marker='o', label='Expt.')
    ax12.plot(v2, g2n, color=cmap(norm(n)))
    if n == 0:
        ax12.scatter(exp_iv_v,exp_iv_in, s=100, facecolors='none', edgecolors='red', marker='o')

    for k in range(n_steps):
        if n == 0:
            ax21.plot(df.time(), df['voltage',k], color='#1f77b4', lw=1)
            ax23.plot(e.time(), e['current',k], color='#1f77b4', lw=1)
        ax22.plot(df.time(), df['current',k], color=cmap(norm(n)), lw=1, label=model_str if k == 0 else '')
...
if args.show == True:
    ...
    filename1 = 'All_figures/Biomarkers-activation-all-models-compare-fit-' + protocol_str + '-iid-noise'
    filename2 = 'All_figures/Activation-all-models-compare-fit-' + protocol_str + '-iid-noise'
