...
x1, x2, y1, y2 = 2900, 3350, 0, 2.3 # specify the limits
...
axins2 = inset_axes(a1, 2, 1.5, loc='lower center') # zoom-factor: 5
x1, x2, y1, y2 = 3430, 3460, 0, 2.9 # specify the limits
...
axins3 = inset_axes(a1, 1.8, 1.4, loc='lower right') # zoom-factor: 5
x1, x2, y1, y2 = 5020, 5650, 0, 2 # specify the limits
...
for n, model in enumerate(models):
    ...
    if n == 0:
        axins.plot(e.time(), e['current'], color='#1f77b4', lw=1, label='Expt.')
        axins2.plot(e.time(), e['current'], color='#1f77b4', lw=1, label='Expt.')
        axins3.plot(e.time(), e['current'], color='#1f77b4', lw=1, label='Expt.')
    axins.plot(d.time(), d['current'], color=colors[n], lw=1, linestyle=linestyles[n], label=model_str)
...
# axins.legend(loc='lower right', fontsize=12, ncol=4)
axins2.legend(fontsize=8)

if args.show == True:
    pl.show()
else:
    filename = 'Complex-AP-models-compare-fit-' + protocol_str + '-iid-noise'
