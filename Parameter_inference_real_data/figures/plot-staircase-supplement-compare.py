...
axins1 = inset_axes(a1, 2.5, 2.75, loc='lower left')
x1, x2, y1, y2 = 1890, 1950, -2.4, 0.75 # specify the limits
...
axins2 = inset_axes(a1, 5.4, 3.4, loc='lower right')
x1, x2, y1, y2 = 4600, 6800, 0.4, 1.9 # specify the limits
...
for n, model in enumerate(models):
    ...
    if n == 0:
        axins1.plot(e.time(), e['current'], color='silver')
        axins2.plot(e.time(), e['current'], color='silver', label='Expt.')
    axins1.plot(d.time(), d['current'], color=cmap(norm(n)))
    axins2.plot(d.time(), d['current'], color=cmap(norm(n)), label=model_str)

axins2.legend(fontsize=8, loc='upper left')

if args.show == True:
    pl.show()
else:
    filename = 'Staircase-all-models-compare-fit-' + protocol_str + '-iid-noise'
